...
//*FUNCTION DAY-NIGHT
const isDayTime = function(dayTime){
  if(dayTime === 1){
...
const getCityApi = async function(city){
  try{
     addLoad()
    //Current weather
    const dataToday = await fetch(`http://api.weatherapi.com/v1/current.json?key=${apiKey2}&q=${city}`)
    const resToday = await dataToday.json()
    console.log(resToday);
    const {current: {temp_c : temperature}} =resToday
    locationTemperature.innerHTML= `${temperature}`
    //ADD Location name in UI
    const {location : {name: name}} =resToday
    locationCity.innerHTML= `${name}`
    //ADD HUMIDITY ,WIND,DAYTIME
    const {current :{humidity: humidity , wind_kph : wind , is_day: day}} = resToday
    isDayTime(day)
    humidityEl.innerHTML = `${humidity} %`
    windEl.innerHTML = `${wind} Km/h`

    //Astronomy - ADD Sunrise/Sunset
    const data = await fetch(`http://api.weatherapi.com/v1/astronomy.json?key=${apiKey2}&q=${city}`)
...
const getCity = async function (lat, lng) {
  try {
    ...
    const { poi : {addr_city: cityGeo , addr_suburb : subCity}}= dataGeo
